...
# Image/tag variables must remain deferred to the Actions shell.
# shellcheck disable=SC2016
grep -Fq '"podman:${IMAGE_NAME}:${DEFAULT_TAG}"' "${build}" \
    || fail "SBOM is not generated from the assembled local image"
