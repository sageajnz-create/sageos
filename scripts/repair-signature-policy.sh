#!/usr/bin/env bash
# SageOS: one-time repair for container signature policy on an EXISTING install.
#
# Why this exists
# ---------------
# The image ships system_files/etc/containers/policy.json, which requires
# sigstoreSigned for ghcr.io/sageajnz-create/sageos. On an existing install,
# ostree performs a 3-way merge of /etc, so a pre-existing (Fedora/Bazzite)
# policy.json already present on the machine wins over the image's copy. The
# result: signing is configured, but verification is NOT enforced at pull time,
# and `sageos-doctor` reports:
#
...
BACKUP="${SAGEOS_REPAIR_BACKUP:-${HOME}/sageos-policy-repair-$(date +%F)}"

die() { printf '\033[1;31mSTOP:\033[0m %s\n' "$*" >&2; exit 1; }
